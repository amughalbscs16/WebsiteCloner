...
def cloner(url):
    ...
    for i in range(0,len(soup.findAll('img'))):
        ...
        soup.findAll('img')[i]['src'] = DownloadFile(soup.findAll('img')[i]['src'], path, files_dict, link_file)
    ...
    for i in range(0,len(soup.findAll('script'))):
        try:
            ...
            if 'src' in str(soup.findAll('script')[i]): 
                ...
                soup.findAll('script')[i]['src'] = DownloadFile(soup.findAll('script')[i]['src'],path,files_dict, link_file)
        except:
            pass
    ...
    for i in range(0,len(soup.findAll('link'))):
        ...
        soup.findAll('link')[i]['href'] = DownloadFile(soup.findAll('link')[i]['href'],path,files_dict, link_file)
